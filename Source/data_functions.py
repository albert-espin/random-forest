...
def convert_categorical_columns_to_numeric(data_frame):
    ...
    data_frame = pd.get_dummies(data_frame, columns=categorical_column_names_without_class)

    # put class column back as last one
    return data_frame[[column for column in data_frame if column not in [class_column_name]] + [class_column_name]]
